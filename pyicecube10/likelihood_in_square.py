...
from .datareader import psf
from .binned_signalness import space_sness
...
def space_signalness(ra, dec, sigma, ra_src, dec_src, R_src, type="analytic"):
        if type == "binned":     
            return space_sness(ra, dec, sigma, ra_src, dec_src, R_src, profile="gaussian")
...
class LikelihoodAnalysisSinglePeriod:
    ...
    def _backgroundness_factory(self):     
        ...
        omegas = 2 * np.sin(np.deg2rad(deltadec)) * np.cos(np.deg2rad(back_bins_s_c)) * np.deg2rad(self.ra_max - self.ra_min)
        ...
        self.events = self.events.query('`RA[deg]` >= @self.ra_min') \
                                 .query('`RA[deg]` <= @self.ra_max')
    ...
    def _en_signalness_factory(self):
        ...
        decs = np.array([#-89.99, -81.87 , -70.335, -64.285, -59.39 , -55.135, -51.295, -47.755,
                        ...
                        #24.84 ,  27.395,  
                        30.01 ,  32.695,  35.465,  38.33 ,  41.315, 44.445,  47.755, 51.295, 
                        #  55.135,  59.39 ,  64.285,  70.335, 81.87, 89.99
                        ])
    ...
    def signalness_func(self, ra, dec, sigma, ra_src, dec_src, R_src, log10_En, gamma):
        return space_signalness(ra, dec, sigma, ra_src, dec_src, R_src, self.sig_method) * self.en_signalness_func(dec_src, gamma, log10_En)
